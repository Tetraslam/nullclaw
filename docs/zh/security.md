...
- `--llm-triage external` — 通过已配置 provider vtable 提交 envelope。默认使用配置中的 primary provider 和 model，可用 `--llm-provider` / `--llm-model` 覆盖。若 provider 是本地的（如 Ollama），envelope 也不会离开本机
...
- 对隐私敏感场景，启用 `--llm-triage external` 前将 primary provider 固定为 `ollama` 等本地 provider
