...
Watcher child agents cannot use the supported `schedule` tool path; only the daemon scheduler owns rearming. This is a capability boundary, not a sandbox: with permissive/yolo shell access, a child could still edit `cron.json` directly. Terminal markers and the choice of verification tool are also model-mediated: a successful irrelevant tool call can satisfy the evidence threshold. Exact trailing-token parsing, rejection of earlier marker text, and a trusted child-process tool-count receipt reduce accidental acceptance but do not independently attest the claimed result.
