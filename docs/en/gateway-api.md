...
To create a durable watcher, send a one-shot agent payload with `repeat_delay`, for example `{"delay":"1m","repeat_delay":"5m","prompt":"Check whether the import completed"}`. `repeat_delay` is valid only with `delay` and `prompt`, without `command`. The daemon scheduler reruns an incomplete or failed check after that positive duration. `/cron/update` can change an existing watcher's cadence with `repeat_delay`. The child must finish with an exact trailing `WATCHER_PENDING`, `WATCHER_SUCCESS`, or `WATCHER_FAILURE` marker. Terminal success or failure is accepted only when the same child turn completed at least one successful tool call. Pending, unverified or missing/malformed terminal markers, empty or marker-only output, execution failures, and failed expected delivery stay silent and rearm. A verified nonempty success or terminal failure strips the marker and removes the job after delivery succeeds; removal without delivery is valid when the configured delivery mode intentionally filters that outcome or is `none`. `/cron` responses expose `watcher` and parsed `repeat_delay_secs` fields, which persist across daemon restarts.

Watcher child agents receive the restricted subagent tool set, without direct scheduling, delegation, spawning, or delivery tools. They must return results only to the scheduler and must not send notifications themselves; only the scheduler owns rearming and terminal delivery. This is a capability boundary, not a sandbox: with permissive/yolo shell access, a child could still edit `cron.json` or invoke an external delivery API directly. Terminal markers and the choice of verification tool are also model-mediated: a successful irrelevant tool call can satisfy the evidence threshold. Exact trailing-token parsing, rejection of earlier marker text, and a trusted child-process tool-count receipt reduce accidental acceptance but do not independently attest the claimed result.
