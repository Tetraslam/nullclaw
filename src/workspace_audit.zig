...
fn readStagedDiff(allocator: Allocator, cwd: []const u8) ![]u8 {
    ...
    const result = process_util.run(allocator, &.{ "git", "diff", "--no-ext-diff", "--src-prefix=a/", "--dst-prefix=b/", "--cached", "--unified=0", "--no-color", "--", "." }, .{
        .cwd = cwd,
...
fn readCommitDiff(allocator: Allocator, cwd: []const u8, commit: []const u8) ![]u8 {
    ...
    const result = process_util.run(allocator, &.{ "git", "show", "--no-ext-diff", "--src-prefix=a/", "--dst-prefix=b/", "--format=", "--unified=0", "--no-color", commit, "--", "." }, .{
        .cwd = cwd,
        .max_output_bytes = MAX_DIFF_BYTES,
    }) catch |err| switch (err) {
        error.FileNotFound => return AuditError.GitUnavailable,
...
fn readRangeDiff(allocator: Allocator, cwd: []const u8, range: []const u8) ![]u8 {
    ...
    const result = process_util.run(allocator, &.{ "git", "diff", "--no-ext-diff", "--src-prefix=a/", "--dst-prefix=b/", "--unified=0", "--no-color", range, "--", "." }, .{
        .cwd = cwd,
        .max_output_bytes = MAX_DIFF_BYTES,
    }) catch |err| switch (err) {
        error.FileNotFound => return AuditError.GitUnavailable,
